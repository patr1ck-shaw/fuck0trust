...
function statusLabel(status) {
  ...
  if (status === "blacklisted") return "已拉黑";
  return "待审批";
}
...
function adminPage(records, message, err) {
  const rows = records.length ? records.map(function (r) {
    ...
    const delConfirm = "return confirm('确定删除该设备申请记录吗？此操作不可恢复。')";
    const blkConfirm = "return confirm('确定拉黑该设备吗？客户端同步状态时会被强制退出。')";
    function act(action, cls, label, onsubmit) {
      return '<form class="inline" method="post" action="/admin/decision"' + (onsubmit ? ' onsubmit="' + onsubmit + '"' : '') + '><input type="hidden" name="deviceId" value="' + id + '"><input type="hidden" name="action" value="' + action + '"><button class="' + cls + '">' + label + '</button></form>';
    }
    return '<tr><td class="' + esc(r.status) + '"><b>' + esc(statusLabel(r.status)) + '</b></td><td><code>' + id + '</code></td><td>' + esc(r.hostname || '-') + '<br>' + esc(r.username || '-') + '</td><td>' + esc(r.note || '-') + '</td><td>' + esc(r.updatedAt || r.requestedAt || '-') + '</td><td><div class="actions">' + act("approve", "approve", "批准") + act("deny", "deny", "拒绝") + act("blacklist", "blacklist", "拉黑", blkConfirm) + act("delete", "delete", "删除", delConfirm) + '</div></td></tr>';
...
async function setStatus(deviceId, status) {
  ...
  const record = Object.assign({}, old, {
    status: status,
    updatedAt: now,
    approvedAt: status === "approved" ? now : old.approvedAt,
    deniedAt: status === "denied" ? now : old.deniedAt,
    blacklistedAt: status === "blacklisted" ? now : old.blacklistedAt
  });
...
async function deleteRecord(deviceId) {
  ...
  const old = await DEVICE_APPROVAL_KV.get(key, "json");
  if (!old) return { ok: false, error: "设备申请不存在" };
  await DEVICE_APPROVAL_KV.delete(key);
...
async function handleDecision(request) {
  ...
  if (!validateDeviceId(id)) return redirect("/admin?error=" + encodeURIComponent("参数错误"));

  if (action === "delete") {
    const r = await deleteRecord(id);
    return redirect("/admin?" + (r.ok ? "msg=" + encodeURIComponent("已删除该设备申请") : "error=" + encodeURIComponent(r.error)));
  }

  const status = action === "approve" ? "approved" : action === "deny" ? "denied" : action === "blacklist" ? "blacklisted" : "";
  if (!status) return redirect("/admin?error=" + encodeURIComponent("参数错误"));
  const r = await setStatus(id, status);
  const okMsg = status === "approved" ? "已批准" : status === "denied" ? "已拒绝" : "已拉黑";
  return redirect("/admin?" + (r.ok ? "msg=" + encodeURIComponent(okMsg) : "error=" + encodeURIComponent(r.error)));
}
...
async function handleRequestApproval(request) {
  ...
  const old = await DEVICE_APPROVAL_KV.get(key, "json");
  // 已被拉黑的设备不允许再次提交申请，直接拒绝
  if (old && old.status === "blacklisted") {
    return json({ ok: false, blacklisted: true, error: "该设备已被拉黑" }, { status: 403 });
  }
...
async function handleStatus(url) {
  ...
  const blacklisted = !!record && record.status === "blacklisted";
  return json({ ok: true, approved: !!record && record.status === "approved", blacklisted: blacklisted, record: record || null });
}
...
async function fetchHandler(request) {
  ...
  try {
    ...
    if (request.method === "POST" && url.pathname === "/api/admin/blacklist") { const e = requireAdmin(request); if (e) return e; const b = await readBody(request); if (!validateDeviceId(b.deviceId)) return json({ ok: false, error: "缺少合法 deviceId" }, { status: 400 }); return json(await setStatus(b.deviceId, "blacklisted")); }
    if (request.method === "POST" && url.pathname === "/api/admin/delete") { const e = requireAdmin(request); if (e) return e; const b = await readBody(request); if (!validateDeviceId(b.deviceId)) return json({ ok: false, error: "缺少合法 deviceId" }, { status: 400 }); return json(await deleteRecord(b.deviceId)); }
